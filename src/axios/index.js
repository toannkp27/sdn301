...
export const clientApi = axios.create({
    ...
    // baseURL: 'http://192.168.6.28:5000/',
    // baseURL: 'http://localhost:5000/',
    // baseURL: 'http://192.168.6.27:5000/',
    timeout: 10000,
});
...
clientApi.interceptors.response.use(
    async function (res) {
        if (res.data.mess === 'access denied') {
            store.dispatch(setToast({ ...listToast[1], detail: 'Bạn không có quyền thực hiện tác vụ này' }))
            return { data: { status: false, mess: 'Bạn không có quyền thực hiện tác vụ này' } }
        }
        return res
    },
    async function (error) {
        store.dispatch(
            setToast({ ...listToast[1], detail: error.mess || 'Đường truyền không ổn định, vui lòng thử lại sau!' }),
        )
        return {
